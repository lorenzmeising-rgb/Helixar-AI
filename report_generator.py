...
def export_report_pdf(
    ...
    # Footer reference to the detailed report. Process comparison now
    # lives in its own sidebar page and produces a separate PDF, so we
    # point users there instead of referencing pages in this document.
    _footer_de = ("Detaillierte Analyse und Quellen auf den folgenden Seiten. "
                  "Mehrere Routen vergleichen: Sidebar → „Prozessvergleich\".")
    _footer_en = ("Detailed analysis and sources on the following pages. "
                  "Compare multiple routes: sidebar → \"Process Comparison\".")
    elems.append(Paragraph(
    ...
    # (Feature A — Routenvergleich removed from the standard report;
    # now lives in a dedicated 'Prozessvergleich' page so the
    # recommendation PDF stays focused on the single chosen route.)
